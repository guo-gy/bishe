...
class MultiModalBalance(nn.Module):
    def __init__(self, vis_dim=512, state_dim=1024, nhead=8, dropout=0.1):
        super().__init__()
        self.vis_norm1 = nn.LayerNorm(vis_dim)
        self.vis_norm2 = nn.LayerNorm(vis_dim)
        self.txt_norm1 = nn.LayerNorm(vis_dim)
        self.txt_norm2 = nn.LayerNorm(vis_dim)
        self.txt_to_vis = nn.MultiheadAttention(vis_dim, nhead,
                                                dropout=dropout)
        self.vis_to_txt = nn.MultiheadAttention(vis_dim, nhead,
                                                dropout=dropout)
        self.vis_ffn = FeedForward(vis_dim, vis_dim * 4, dropout)
        self.txt_ffn = FeedForward(vis_dim, vis_dim * 4, dropout)
        self.state_proj = nn.Linear(state_dim, vis_dim)
        self.state_update = nn.Linear(vis_dim, state_dim)
        self.gate = nn.Sequential(nn.Linear(vis_dim * 2, vis_dim),
                                  nn.ReLU(True), nn.Linear(vis_dim, vis_dim),
                                  nn.Sigmoid())

    def forward(self, vis, txt, state, pad_mask):
        b, c, h, w = vis.size()
        vis_tokens = vis.reshape(b, c, -1).permute(2, 0, 1)
        txt_tokens = txt.permute(1, 0, 2)

        txt_residual = self.vis_to_txt(query=self.txt_norm1(txt_tokens),
                                       key=self.vis_norm1(vis_tokens),
                                       value=self.vis_norm1(vis_tokens))[0]
        txt_tokens = txt_tokens + txt_residual
        txt_tokens = txt_tokens + self.txt_ffn(self.txt_norm2(txt_tokens))

        vis_residual = self.txt_to_vis(query=self.vis_norm1(vis_tokens),
                                       key=self.txt_norm1(txt_tokens),
                                       value=self.txt_norm1(txt_tokens),
                                       key_padding_mask=pad_mask)[0]
        vis_tokens = vis_tokens + vis_residual
        vis_tokens = vis_tokens + self.vis_ffn(self.vis_norm2(vis_tokens))

        txt_out = txt_tokens.permute(1, 0, 2)
        txt_out = txt_out.masked_fill(pad_mask.unsqueeze(-1), 0.)
        vis_out = vis_tokens.permute(1, 2, 0).reshape(b, c, h, w)

        txt_global = masked_mean(txt_out, pad_mask)
        vis_global = vis_out.flatten(2).mean(-1)
        state_context = txt_global + self.state_proj(state)
        gate = self.gate(torch.cat([vis_global, state_context], dim=-1))
        vis_out = vis + vis_out * gate.unsqueeze(-1).unsqueeze(-1)
        state = state + self.state_update(txt_global)

        align_loss = 1 - F.cosine_similarity(
            F.normalize(vis_global, dim=-1),
            F.normalize(state_context, dim=-1),
            dim=-1)
        align_loss = align_loss.mean()
        return vis_out, txt_out, state, align_loss
